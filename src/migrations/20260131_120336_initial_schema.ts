...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`at_avatar\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`at_avatar_order_idx\` ON \`at_avatar\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`at_avatar_parent_id_idx\` ON \`at_avatar\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`at_avatar_path_idx\` ON \`at_avatar\` (\`_path\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_users_v\` (
  	...
  	\`_uuid\` text,
  	FOREIGN KEY (\`image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`_at_avatar_v\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`_users_v_order_idx\` ON \`_users_v\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`_users_v_parent_id_idx\` ON \`_users_v\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_users_v_image_idx\` ON \`_users_v\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`_at_avatar_v\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`_at_avatar_v_order_idx\` ON \`_at_avatar_v\` (\`_order\`);`)
  await db.run(
    sql`CREATE INDEX \`_at_avatar_v_parent_id_idx\` ON \`_at_avatar_v\` (\`_parent_id\`);`,
  )
  await db.run(sql`CREATE INDEX \`_at_avatar_v_path_idx\` ON \`_at_avatar_v\` (\`_path\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`at_avatar\`;`)
  ...
  await db.run(sql`DROP TABLE \`_at_avatar_v\`;`)
